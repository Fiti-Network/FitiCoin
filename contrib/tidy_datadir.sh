...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Fiticoin database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Fiticoin datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Fiticoin datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Fiticoin 0.7 datadir."
    ;;
  3)
    echo "Detected Fiticoin pre-0.8 datadir."
    ;;
  4)
    echo "Detected Fiticoin 0.8 datadir."
    ;;
esac
