...
*Setup instructions for a Gitian build of Fiticoin Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Fiticoin
...
- [Building Fiticoin Core](#building-fiticoin-core)
...
Clone the git repositories for fiticoin and Gitian.
...
in the fiticoin repository under 'Fetch and create inputs' to install sources which require
manual intervention. Also optionally follow the next step: 'Seed the Gitian sources cache
and offline git repositories' which will fetch the remaining files required for building
offline.

Building Fiticoin Core
----------------

To build Fiticoin Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the fiticoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/fiticoin/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/fiticoin-project/fiticoin
...
For example:
```bash
URL=https://github.com/thrasher-/fiticoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit fiticoin=${COMMIT} --url fiticoin=${URL} ../fiticoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit fiticoin=${COMMIT} --url fiticoin=${URL} ../fiticoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit fiticoin=${COMMIT} --url fiticoin=${URL} ../fiticoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the fiticoin git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../fiticoin/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/fiticoin
SIGPATH=/some/root/path/fiticoin-detached-sigs

./bin/gbuild --url fiticoin=${BTCPATH},signature=${SIGPATH} ../fiticoin/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/fiticoin-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/fiticoin-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/fiticoin-osx-build.assert
```
...
[fiticoin-project/gitian.sigs.ltc](https://github.com/fiticoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
