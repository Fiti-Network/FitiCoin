Fiticoin Core version 0.14.2 is now available from:

  <https://download.fiticoin.org/fiticoin-0.14.2.0/>
...
To receive security and update notifications, please subscribe to:
  ...
  <https://groups.google.com/forum/#!forum/fiticoin-dev>

Compatibility
==============

Fiticoin Core is extensively tested on multiple operating systems using
...
Fiticoin Core should also work on most other Unix-like systems but is not
...
Fiticoin Core now supports P2SH addresses beginning with M on mainnet and Q on testnet.
...
This only affects users that have explicitly enabled UPnP through the GUI setting or through the -upnp option, as since the last UPnP vulnerability (in Fiticoin Core 0.10.4) it has been disabled by default.
...
- http://testnet.fiticointools.com
- http://testnet.thrasher.io

Developers who require the new testnet blockchain paramaters can find them [here](https://github.com/fiticoin-project/fiticoin/blob/master/src/chainparams.cpp#L220).
...
Fiticoin Core has supported automatically pruning the blockchain since 0.13.2. Pruning
...
now ZMQ can be used on Windows. Please see [this document](https://github.com/fiticoin-project/fiticoin/blob/master/doc/zmq.md) for
...
When Fiticoin Core is out-of-sync on startup, a semi-transparent information
...
Commands sent over the JSON-RPC interface and through the `fiticoin-cli` binary
can now use named arguments. This follows the [JSON-RPC specification](http://www.jsonrpc.org/specification)
for passing parameters by-name with an object.

`fiticoin-cli` has been updated to support this by parsing `name=value` arguments
when the `-named` option is given.

Some examples:

    src/fiticoin-cli -named help command="help"
    src/fiticoin-cli -named getblockhash height=0
    src/fiticoin-cli -named getblock blockhash=000000000019d6689c085ae165831e934ff763ae46a2a6c172b3f1b60a8ce26f
    src/fiticoin-cli -named sendtoaddress address="(snip)" amount="1.0" subtractfeefromamount=true
...
The Alert System was disabled and deprecated in Fiticoin Core 0.10.4 and removed in 0.13.2. 
...
----------------------
 ...
 - A new RPC command `getmemoryinfo` has been added which will return information
   about the memory usage of Fiticoin Core. This was added in conjunction with
...
here, originally written for Bitcoin but can also be used for Fiticoin:
